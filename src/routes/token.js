...
router.post(
  ...
  handleError(TokenSchema),
  async (req, res) => {
    let shopAlias,
      productFakeVariantId,
    ...
    try {
      ...
      const objectToken = await dbRepository.validateToken(
        shopAlias,
        email,
        token
      )
      ...
      let subscriptionData = await subscriptionImp.getSubscription(
        email,
        subscription,
        true
      )
      if (!subscriptionData)
        throw new Error("It is not possible to cancel the subscription")
      if (subscriptionData.cyclesCompleted > 1)
        throw new Error("The subscription have more than 1 cycle completed")
      if (!subscriptionData.SubscriptionLines.length) {
        subscriptionData = await subscriptionImp.getSubscription(
          email,
          subscription,
          false
        )
      ...
      if (
        subscriptionData.ShippingAddress.province.toUpperCase() === "CALIFORNIA"
      ) {
        const subscriptionCancelled = await subscriptionImp.cancelSubscription(
          cancelSessionId,
          subscription
        )
      ...
      let lineItems = (
        await shopifyImp.getLineItemsByOrder(
          subscriptionData.originOrder.platformId
        )
      ).map((e) => e.node)
      ...
      for (let upsell of upsells) {
        ...
        console.log(
          `Upsell: Price ${eachUpsell.ProductVariant.title} - PriceWithoutDiscount ${eachUpsell.priceWithoutDiscount} - Quantity${eachUpsell.quantity}`
        )
        quantity +=
          getPriceDifference(
            eachUpsell.ProductVariant.price,
            eachUpsell.priceWithoutDiscount
          ) * eachUpsell.quantity

        console.log(`${time_ms} - EachUpsell: ${JSON.stringify(eachUpsell)}`)
        console.log(
          `${time_ms} - Quantity: ${eachUpsell.ProductVariant.price}, ${eachUpsell.priceWithoutDiscount}, ${eachUpsell.quantity}, ${quantity}`
        )
      }
      ...
      const oneTimeBySubscriptionMetafieldQuery = noUpsells
        .map(
          (e) =>
            `(metafields.custom.${productSubscriptionMetafieldKey}:${e.product.id
              .split("/")
              .pop()} AND price:>0 AND -product_type:Gift)`
        )
        .join(" OR ")
      const oneTimeProducts = (
        await shopifyImp.oneTimesBySubscriptionMetafield(
          productSubscriptionMetafieldKey,
          oneTimeBySubscriptionMetafieldQuery
        )
      ).map((e) => ({
        ...
        variants: e.node.variants.edges
          .map((i) => i.node)
          .map((i) => {
            i.title = i.title.replace(/\s/g, "").toLowerCase()
            return i
          }),
      }))

      console.log(
        `${time_ms} - oneTimeProducts: ${JSON.stringify(oneTimeProducts)}`
      )

      let productSub = null
      let productOneTime = null
      for (let oneTime of oneTimeProducts) {
        productSub = lineItems.find(
          (e) => e.product.id === oneTime.subscriptionProductId
        )
        ...
        if (oneTime.variants.length === 1) {
          ...
          productOneTime = oneTime.variants.find(
            (e) => e.title === productSub.variant.title
          )
        ...
        console.log(
          `OneTime: Price ${productOneTime.price} - Quantity ${productSub.quantity}`
        )
        quantity +=
          getPriceDifference(productOneTime.price, productSub.variant.price) *
          productSub.quantity
        console.log(
          `${time_ms} - EachOneTime: ${JSON.stringify(productOneTime)}`
        )
        console.log(
          `${time_ms} - Quantity: ${quantity}, ${productOneTime.price}, ${productSub.variant.price}, ${productSub.quantity}`
        )
      }

      if (quantity <= 0)
        throw new Error("It was not possible to calculate the price difference")
      ...
      await dbRepository.saveDraftOrder(
        shopAlias,
        draftOrderId,
        subscription,
        cancelSessionId
      ...
      res.json({
        message:
          "To finalize the subscription cancellation process please pay the draft order that has been created",
      })
    } catch (err) {
      console.log(err)
...
router.post("/address/validate", handleError(TokenSchema), async (req, res) => {
  ...
  try {
    ...
    const expirationDateUpdated = await dbRepository.updateTokenExpirationDate(
      shopAlias,
      email,
      token
    )
    ...
    const errorFullMessage = `${errorShop}${errorData}${errorDescription}${errorRoute}`
    const errorTitle =
      "🔴 ❌ ERROR: Error while trying to validate token to update address"
    messageImp.toCancelSubscriptionErrors(errorFullMessage, errorTitle)
  }
})
