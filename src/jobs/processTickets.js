...
Another task is to determine if the message is spam or not, create a key called "spam" with a value of true as a boolean not string.
When determining if the message is spam, use these guidelines:
Mark as spam ("spam": true) if the message contains irrelevant content, excessive promotional language, suspicious links, or unrelated services/products not associated with the listed brands.
Mark as spam if the message seems autogenerated, contains nonsense text, repeated patterns, or lacks a clear intent to collaborate.
Mark as spam if it includes generic mass messages (e.g., “Hello dear, click here for a great opportunity”), poor grammar suggesting bots, or irrelevant language to the brand context.
Otherwise, mark "spam": false.
If a message is spam, do not fill any of the other fields except "spam".
`
...
const processOneTicket = async (ticketRow) => {
  try {
    ...
    if(customerData.spam === false || customerData.spam === "false") {
      ...
      await slack.postMessage('C09176CKX9A', `Ticket https://b2cresponse.gorgias.com/app/ticket/${ticketRow.ticket_id} is considered spam, marked as completed`);
      await gorgias.updateTicketStatus(ticketRow.ticket_id, 'closed');
    }
